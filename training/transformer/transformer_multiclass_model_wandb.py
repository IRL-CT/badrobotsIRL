...
def train_early_fusion(df, config):
    ...
    for fold in range(5):
        ...
        cm = confusion_matrix(y_test_class_indices, y_pred, labels=range(4))
        ...
        del X_test_sequences, y_test_sequences, y_test_sequences_cat
...
def train_intermediate_fusion(modality_dfs, config):
    ...
    for fold in range(5):
        ...
        session_test = first_modality_splits[15]
        ...
        del y_train_sequences, y_val_sequences, y_test_sequences, y_test_sequences_cat
        del y_predict_probs, y_predict_probs_clean
        tf.keras.backend.clear_session()
        gc.collect()
...
def train_late_fusion(modality_dfs, config):
    ...
    for fold in range(5):
        ...
        if len(y_train_sequences.shape) == 1 or y_train_sequences.shape[1] == 1:
            num_classes = 4
            y_train_sequences_cat = to_categorical(y_train_sequences, num_classes=4)
            y_val_sequences_cat = to_categorical(y_val_sequences, num_classes=4)
            y_test_sequences_cat = to_categorical(y_test_sequences, num_classes=4)
        else:
        ...
        del y_train_sequences_cat, y_val_sequences_cat, y_test_sequences_cat
        del y_predict_probs, y_predict_probs_clean
        tf.keras.backend.clear_session()
        gc.collect()
...
def train():
    ...
    if feature_set in ["curated_features_v5_100fps", "tsfresh", "catch22"]:
        ...
        df_base = pd.read_csv(os.path.join(DATA_DIR, "feature_sets", "select_k_best_allparticipants_100fps_multiclass_label.csv"))
    else:
...
def main():
    ...
    sweep_config = {
        'method': 'random',
        'name': 'brirl_transformer_multiclass',
        'parameters': {
