...
from gru_multiclass_single_modality import train_single_modality_model
...
def train():
    ...
    wandb.init()
    config = wandb.config
    # wandb.config.update({"modality": "pose_gemini"}, allow_val_change=True)
    print(config)
    ...
    if feature_set not in NO_MODALITY_SELECTION_SETS:
        ...
        # Embeddings must be aligned to the windowed rows via last_positions
...
def main():
    ...
    sweep_config = {
        'method': 'random',
        'name': 'brirl_gru_multiclass_inter',
        'parameters': {
