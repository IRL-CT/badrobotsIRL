...
def train_early_fusion(df, config):
    ...
    for fold_no in range(len(unique_sessions)):
        ...
        print("Unique labels val:", np.unique(y_val))
    ...
    for fold_no in range(len(unique_sessions)):
        ...
        for modality_key in modality_keys:
            ...
            if splits[modality_key] is None:
                print(f"[Fold {fold_no}] Invalid split for modality {modality_key} or participant. Skipping...")
                splits_valid = False
                break
...
def main():
    ...
    sweep_config = {
        'method': 'random',
        'name': 'lstm_binary_intra_train_rat_fixed_test',
        'parameters': {
    ...
    sweep_id = wandb.sweep(sweep=sweep_config, project="lstm_binary_intra_train_rat_fixed_test")
