...
def train_early_fusion(df, config):
    ...
    for fold in range(5):
        ...
        session_test = df.iloc[:, 1].values  # participant column
        test_indices = df[df.iloc[:, 1].isin(
            df.iloc[:, 1].unique()  # will be filtered by create_data_splits
        )].index
        # Reconstruct session_test from the dataframe based on test fold
        # We need to get sessions aligned with y_test
        # Since create_data_splits doesn't return sessions, extract from df
        ...
        session_test = all_sessions[test_idx]
...
def train_intermediate_fusion(modality_dfs, config):
    ...
    for fold in range(5):
        ...
        first_df = modality_dfs[modality_keys[0]]
        excluded_participants = ["p24nodbot"]
        df_filtered = first_df[~first_df['participant'].isin(excluded_participants)].reset_index(drop=True)
        all_sessions = df_filtered['participant'].values
        fold_sessions = df_filtered['participant'].unique()
        np.random.seed(42)
        np.random.shuffle(fold_sessions)
